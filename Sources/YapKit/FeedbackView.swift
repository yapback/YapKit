...
public struct FeedbackView: View {
    ...
    @State private var attachmentLoadingState: AttachmentLoadingState = .idle
    ...
    public var body: some View {
        NavigationStack {
            Form {
                ...
                if case .error(let errorMessage) = submissionState {
                    Section {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    if case .uploading(let progress) = submissionState {
                        HStack(spacing: 6) {
                            ProgressView()
                                .controlSize(.small)
    ...
    private func submit() {
        ...
        Task {
            do {
                ...
                // If we have attachments, use the upload flow
                if !attachments.isEmpty {
                    await MainActor.run {
                        submissionState = .uploading(0)
                    }
                    ...
                    _ = try await service.submit(
                        ...
                        Task { @MainActor in
                            submissionState = .uploading(progress.overallProgress)
                        }
                    }
                } else {
                    await MainActor.run {
